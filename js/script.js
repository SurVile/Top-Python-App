...
buy_item.forEach(item => { // перебираем все кнопки Купить
    item.addEventListener('click', () => { // перехватываем событие нажатия на любую кнопку Купить
        let button_block_info = item.parentNode; // создаем переменную с информацие о блоке, в которой находится любая кнопка Купить
        let article_name = button_block_info.parentNode.getAttribute('name'); // создаем переменную с тазванием карточки товара, в которой находится любая кнопка Купить
    
        let article_data = { // содаем объект (не класса) с передаваемой информацией
            data_type: 'buy_item', // вид/обозначение передаваемой информации
            data_name: article_name // название товара
        }
        tg.sendData(JSON.stringify(article_data)); // преобразуем объект с передаваемой инфомацией в JSON-строку
